...
import { toApiTrackPackage } from "@/lib/server/api-projects";
import { getProjectForUser } from "@/lib/server/projects";

type ProjectTrackRouteContext = {
  params: Promise<{
    projectId: string;
  }>;
};

export async function GET(request: Request, context: ProjectTrackRouteContext) {
  ...
  return apiSuccess(toApiTrackPackage(project));
}
