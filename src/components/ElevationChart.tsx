...
export default function ElevationChart() {
  ...
  const chartData = useMemo(() => {
    ...
    const linePath = samples
      .map(({ d, z }, i) => `${i === 0 ? "M" : "L"}${toX(d).toFixed(2)},${toY(z).toFixed(2)}`)
      .join(" ");

    const firstX = toX(samples[0].d);
    const lastX = toX(samples[samples.length - 1].d);
    const baselineY = (PAD_TOP + PLOT_H).toFixed(2);
    const fillPath = linePath + ` L${lastX.toFixed(2)},${baselineY} L${firstX.toFixed(2)},${baselineY} Z`;
    ...
    for (let z = yTickStart; z <= maxZ + yStep * 0.01; z += yStep) {
      if (z >= minZ - 0.001 && z <= maxZ + 0.001) yTicks.push({ z, label: z.toFixed(1) });
    }

    const minSample = samples.reduce((a, s) => (s.z < a.z ? s : a), samples[0]);
    const maxSample = samples.reduce((a, s) => (s.z > a.z ? s : a), samples[0]);

    return { linePath, fillPath, totalDist, rawMinZ, rawMaxZ, toX, toY, xTicks, yTicks, minSample, maxSample };
  }, [path]);

  if (!path || !chartData) {
    return (
      <div className="flex items-center justify-center border-t border-border bg-card/50 px-4 py-4 text-xs text-muted-foreground shrink-0">
        No race line selected
      </div>
    );
  }

  const { linePath, fillPath, totalDist, rawMinZ, rawMaxZ, toX, toY, xTicks, yTicks, minSample, maxSample } = chartData;

  return (
    <div className="border-t border-border bg-card/50 px-3 py-3 shrink-0">
      ...
      <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} width="100%" height={VIEW_H} className="overflow-visible">
        <defs>
        ...
        {yTicks.map(({ z }) => (
          <line key={z}
            x1={PAD_LEFT} y1={toY(z).toFixed(2)}
            x2={PAD_LEFT + PLOT_W} y2={toY(z).toFixed(2)}
            stroke="currentColor" strokeOpacity={0.1} strokeWidth={0.5} strokeDasharray="3 3" />
        ))}

        <path d={fillPath} fill="url(#elev-fill)" clipPath="url(#elev-clip)" />
        <path d={linePath} fill="none" stroke="var(--color-primary)"
          strokeWidth="1.5" strokeLinejoin="round" strokeLinecap="round"
          clipPath="url(#elev-clip)" />

        {minSample.d !== maxSample.d && (
          <>
            <circle cx={toX(minSample.d)} cy={toY(minSample.z)} r="3"
              fill="var(--color-background)" stroke="var(--color-primary)" strokeWidth="1.5" />
            <text x={toX(minSample.d)} y={toY(minSample.z) + 10}
        ...
        <circle cx={toX(maxSample.d)} cy={toY(maxSample.z)} r="3"
          fill="var(--color-primary)" stroke="var(--color-background)" strokeWidth="1.5" />
        <text x={toX(maxSample.d)} y={toY(maxSample.z) - 5}
          textAnchor="middle" fontSize="8" fill="var(--color-primary)" fontWeight="600">
        ...
        {xTicks.map(({ d, label }) => (
          <g key={d}>
            <line x1={toX(d)} y1={PAD_TOP + PLOT_H} x2={toX(d)} y2={PAD_TOP + PLOT_H + 3}
              stroke="currentColor" strokeOpacity={0.2} strokeWidth={1} />
            <text x={toX(d)} y={VIEW_H - 4} textAnchor="middle" fontSize="8"
        ...
        {yTicks.map(({ z, label }) => (
          <g key={z}>
            <line x1={PAD_LEFT - 3} y1={toY(z)} x2={PAD_LEFT} y2={toY(z)}
              stroke="currentColor" strokeOpacity={0.2} strokeWidth={1} />
            <text x={PAD_LEFT - 5} y={toY(z) + 3} textAnchor="end" fontSize="8"
              fill="currentColor" fillOpacity={0.5}>{label}</text>
          </g>
        ))}
      </svg>
