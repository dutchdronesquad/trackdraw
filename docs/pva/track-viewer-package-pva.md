...
- [x] Isolated spike page built outside the Next.js app shell — `src/app/dev/viewer-spike/`, no `EditorShell`/`Header`/`MobilePanels`/auth; extracted viewer lived at `packages/viewer/src/`, now split out to [`dutchdronesquad/track-viewer`](https://github.com/dutchdronesquad/track-viewer)
