...
- `@trackdraw/viewer`: framework-neutral mount API (`createTrackDrawViewer`), ESM npm build plus a static browser build from one codebase/release. Originally built inside this repo at `packages/viewer/` (Phase 0 license/NOTICE scaffold, Phase 1 source extraction consumed via a dev-only TypeScript path alias, Phase 2 (#860) `package.json`/`tsup` build tooling). Issue [#870](https://github.com/dutchdronesquad/trackdraw/issues/870) vendored the package's remaining `@/...` app-internal imports, making `packages/viewer/src` fully self-contained. **Repo split completed** ([#871](https://github.com/dutchdronesquad/trackdraw/issues/871)): the package now lives in its own repository, [`dutchdronesquad/track-viewer`](https://github.com/dutchdronesquad/track-viewer) (deliberately without "trackdraw" in the name, to read as a standalone, brand-neutral project rather than a TrackDraw-only tool; the npm package stays `@trackdraw/viewer`). The directory's git history was preserved via `git subtree split`. Reasoning: the AGPL/Apache-2.0 license boundary between this repo and the package was not previously structurally enforced — issue #870 existed precisely because the boundary had already leaked (the package transitively pulled in AGPL-licensed app code via `@/...` imports). A separate repository makes that boundary a structural fact instead of a code-review discipline, which matters more than the monorepo convenience of colocated versioned releases (git tags/a scoped `package.json` version work fine either way). This repo now consumes it as a pinned git dependency (`"@trackdraw/viewer": "git+https://github.com/dutchdronesquad/track-viewer.git#<sha>"` in `package.json` — the explicit `git+https://` form, not the `github:owner/repo` shorthand, so the intended transport is unambiguous) rather than a workspace/source alias — not yet published to npm, so a git dependency is the interim path; the target repo's own `prepare` lifecycle script builds `dist/` automatically on install. `package-lock.json` still records this dependency's `resolved` URL in `git+ssh://` form regardless of the requested protocol - that's `hosted-git-info`'s standard canonical identity form for any GitHub-hosted git dependency, not the literal fetch transport (verified: installs succeed with no SSH key present, locally and in CI). The per-file Apache-2.0 header comments added across ~70 files during #870's vendoring were dropped in favor of a single root `LICENSE`, now that the package is a standalone single-license repository.
...
- [x] Apache-2.0 `LICENSE`/`NOTICE`/header text drafted for the viewer package (now [`LICENSE`](https://github.com/dutchdronesquad/track-viewer/blob/main/LICENSE)/[`NOTICE.md`](https://github.com/dutchdronesquad/track-viewer/blob/main/NOTICE.md) in the split-off `dutchdronesquad/track-viewer` repo)
...
- [x] Isolated spike page built outside the Next.js app shell — `src/app/dev/viewer-spike/`, no `EditorShell`/`Header`/`MobilePanels`/auth; extracted viewer lived at `packages/viewer/src/`, now split out to [`dutchdronesquad/track-viewer`](https://github.com/dutchdronesquad/track-viewer). Removed once its purpose (proving extraction) was fulfilled and independently verified by that repo's own build/test/CI - it was the only place in this app still rendering the full `TrackViewer` component, and kept `@trackdraw/viewer`'s three.js/Konva-heavy root entry reachable from this app.
...
- [x] Multi-instance isolation confirmed (no shared editor store) — confirmed by construction (no `useEditor`/store import anywhere in the viewer's `src/**`) and by the spike page's two concurrently-rendered instances; details in the [findings doc](../research/in-progress/track-viewer-extraction-spike-findings.md#multi-instance-isolation)
- [x] Finalized display-metadata allowlist and `required_viewer` versioning rule recorded — implementation in [`dutchdronesquad/track-viewer`](https://github.com/dutchdronesquad/track-viewer)'s `src/snapshot/` and [`src/lib/track/viewer-snapshot.ts`](../../src/lib/track/viewer-snapshot.ts); rule recorded in the [findings doc](../research/in-progress/track-viewer-extraction-spike-findings.md#allowlist-and-required_viewer-versioning--finalized)
...
- [x] ESM + static builds published from one release pipeline — now built in [`dutchdronesquad/track-viewer`](https://github.com/dutchdronesquad/track-viewer) (`npm run build`, `tsup`), producing `dist/*.js` (multi-entry ESM) and `dist/static/trackdraw-viewer.global.js` + `trackdraw-viewer.css` (self-contained IIFE + compiled Tailwind, for hosts with no bundler). Not published to any registry — deliberately out of scope, see the PVA's "New Surfaces" section. `.d.ts` type declarations ship via a separate `tsc --declaration` pass (viable once the package's `rootDir` no longer reached outside its own `src/`, after the #871 split) — the earlier known gap here is closed.
...
- [x] `viewer-snapshot` route implemented and schema-validated — `GET /api/v1/projects/[projectId]/viewer-snapshot`; snapshots are validated against `@trackdraw/viewer/snapshot/schema`'s Zod schema plus a `MAX_VIEWER_SNAPSHOT_BYTES` serialized-size cap before ever leaving `toViewerDesignSnapshot()`.
...
- [x] Repository split completed ([#871](https://github.com/dutchdronesquad/trackdraw/issues/871)) — see the "New Surfaces" section above.
