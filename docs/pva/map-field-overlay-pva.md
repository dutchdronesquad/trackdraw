...
- choose hybrid storage for v1
- keep overlay out of share/export in v1
- treat this as an editor-first feature with signed-in cross-device continuity

This document is intended to be strong enough for a product go-ahead, not just for continued brainstorming.
...
This document recommends choosing the second path for v1.
...
These are not optional cleanup details. They materially affect the model.

### Why Not Device-Local Only

Device-local only would be faster, but it weakens one of the clearest reasons this feature matters. If a user aligns a venue map on desktop and later opens the same signed-in project on mobile at the venue, the overlay should still be there.

For that reason, the product should not treat local-only storage as the primary v1 direction.

## Storage Direction

The product model should assume:

- overlay belongs to one project
- logged-out use stores the image locally
- signed-in use stores a synced asset reference for the account-backed project
- replacement and removal remain cheap operations from the user's perspective

The product should also define failure behavior clearly:

- if an image is too large, fail clearly
- if upload or sync fails, the project still opens and remains editable
- if an overlay asset cannot be restored, the project opens without the overlay and explains that state
...
The first version should reuse the existing media/storage direction for signed-in projects rather than inventing a second asset system just for overlays.

Recommended boundary even in the hybrid model:

- overlay asset sync only for signed-in account-backed projects
- no gallery or share-time overlay asset fetch path in v1
- no automatic inclusion of overlay assets in ordinary published output

This keeps the backend scope narrower than “general media platform work” while still enabling cross-device project continuity where it matters.
...
- hybrid signed-in sync is the chosen v1 direction
...
The first engineering slice that should actually be built is:

1. add overlay metadata to `TrackDesign`
2. render one overlay in the editor
3. support local add/replace/remove/move/scale/opacity/lock
4. keep export/share unaware of overlays

Only after that slice is stable should the signed-in asset sync path be wired in.

This keeps the feature shippable in layers without losing the chosen v1 direction.

## Recommended Next Build Target

The next build target should be a first overlay slice with the hybrid storage direction locked first:

- one image overlay
- logged-out local storage plus signed-in synced asset storage
...
This keeps the feature aligned with the chosen product direction without overgeneralizing media handling for the whole app.

## Current Codebase Fit

Relevant current boundaries:

- project document types: [src/lib/types.ts](/Users/klaas/Projecten/trackdraw/src/lib/types.ts)
- design normalization and serialization: [src/lib/track/design.ts](/Users/klaas/Projecten/trackdraw/src/lib/track/design.ts)
- local project persistence: [src/lib/projects.ts](/Users/klaas/Projecten/trackdraw/src/lib/projects.ts)
- cloud project save/load: [src/app/api/projects/route.ts](/Users/klaas/Projecten/trackdraw/src/app/api/projects/route.ts) and [src/lib/server/projects.ts](/Users/klaas/Projecten/trackdraw/src/lib/server/projects.ts)
- account sync orchestration: [src/components/editor/useAccountProjectSync.ts](/Users/klaas/Projecten/trackdraw/src/components/editor/useAccountProjectSync.ts)
- editor store and track mutations: [src/store/editor.ts](/Users/klaas/Projecten/trackdraw/src/store/editor.ts)
...
Keep [src/app/api/projects/route.ts](/Users/klaas/Projecten/trackdraw/src/app/api/projects/route.ts) focused on saving project JSON.
...
Extend the editor track actions in [src/store/editor.ts](/Users/klaas/Projecten/trackdraw/src/store/editor.ts) with narrow overlay actions such as:
...
- [src/components/canvas/editor/TrackCanvas.tsx](/Users/klaas/Projecten/trackdraw/src/components/canvas/editor/TrackCanvas.tsx)
...
The likely coordination point is [src/components/editor/useAccountProjectSync.ts](/Users/klaas/Projecten/trackdraw/src/components/editor/useAccountProjectSync.ts).
...
- [src/lib/schema.d.ts](/Users/klaas/Projecten/trackdraw/src/lib/schema.d.ts)
- [src/lib/track/design.ts](/Users/klaas/Projecten/trackdraw/src/lib/track/design.ts)
- [src/store/editor.ts](/Users/klaas/Projecten/trackdraw/src/store/editor.ts)
- [src/components/canvas/editor/TrackCanvas.tsx](/Users/klaas/Projecten/trackdraw/src/components/canvas/editor/TrackCanvas.tsx)
- [src/components/editor/useAccountProjectSync.ts](/Users/klaas/Projecten/trackdraw/src/components/editor/useAccountProjectSync.ts)
- [src/lib/projects.ts](/Users/klaas/Projecten/trackdraw/src/lib/projects.ts)

## First Build Order

The first engineering order should be:

1. add `design.overlay`
2. add local overlay blob helpers
3. render one overlay in the editor
...
That sequence keeps the feature shippable in layers while still landing on the chosen hybrid v1.
