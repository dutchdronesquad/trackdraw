...
| Applies from    | The first production event carrying `contract_version = "1.0.0"`; version `1.1.0` preserves that measurement history |
...
| `EVT-012` | `export.failed`                    | shipped | An attempted export fails before a usable artifact is generated         | `format`: `png`, `svg`, `json`, `pdf`, `webm`, `race_pack`, `velocidrone`, `render_3d`; `category`: the `EVT-011` category enum; `reason`: `asset_load_failed`, `canvas_unavailable`, `flight_path_failed`, `font_load_failed`, `invalid_design`, `recording_failed`, `rendering_failed`, `serialization_failed`, `track_path_missing`, `unsupported_browser`, `unknown`; `surface`: literal `editor` | Once per failed export                                                       |
...
- `EVT-012` replaces `EVT-011` for export failures emitted by contract `1.1.0` clients. Its format and reason are closed technical enums derived locally from known failure boundaries; they never contain exception text, filenames, track content, or other dynamic values. Contract `1.0.0` export failures remain valid historical `EVT-011` rows and show no format or reason detail.
...
| `MTR-010` | Categorized failure rate     | `EVT-011` count for one `operation` and `category`, plus `EVT-012` for export failures from contract `1.1.0` clients | Successful terminal events plus the applicable failure event for the same operation            | 7 complete days                               | Per operation: first complete UTC day with compliant failure-event and success-event coverage            | 30 terminal operation outcomes                     | `not_started`       | TrackDraw maintainers |
...
- `MTR-010` may be published only for operations with a defined success event: import → `EVT-005`, export → `EVT-006`, share create → `EVT-008`, gallery publish → `EVT-009`, share view → `EVT-007`. Export failures use legacy `EVT-011` rows for contract `1.0.0` and `EVT-012` rows for contract `1.1.0`; one attempt emits only one failure event. Editor load and project save remain failure counts until success events are defined in a later contract version; no rate may be fabricated for them.
...
- `1.1.0` — Adds `EVT-012` with closed export format and technical-reason enums, preserves exact server timestamps for authorized operational drilldown, and keeps contract `1.0.0` export failures as category-only history.
