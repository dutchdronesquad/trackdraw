...
The shared part is rendering and course data. Event records, credentials, organizer permissions, storage, and refresh scheduling remain host responsibilities. A separate generic integration SDK is unnecessary until duplicated adapter work justifies it.
...
Scope the saved connection and all project-list/import actions to the appropriate organizer or organization permissions in FPVScores. The existing TrackDraw permission allows reading the key owner's projects; it is not an event-specific or single-project grant. Never expose that private project list to public visitors or unrelated organizers.
...
- Use explicit container sizing, locale, theme, and unit inputs. No host-router mutation, host-wide CSS reset, implicit persistence, account UI, or analytics callbacks to TrackDraw.
...
With FPVScores, confirm the frontend integration entry, server-side connection storage, organizer/organization permissions, event placement, and snapshot retention. Agree that attaching a private TrackDraw project makes that snapshot visible to the event audience.

The repository currently declares `AGPL-3.0-only` in [package.json](../../../package.json) and [NOTICE](../../../NOTICE.md). Decide the viewer package's intended distribution terms and asset inventory before committing to partner npm adoption; this research does not change licensing or presume a permissive license.

For RotorHazard, confirm plugin ownership and supported versions. FPVScores can proceed first without waiting for the finished RotorHazard plugin, provided the common artifact is locally serveable from the start.
...
### 4. RotorHazard Local And Cloud Adapter
...
The recommended architecture is concrete enough for host alignment, but these choices remain open before a PVA:

- FPVScores owner and stack, integration settings scope, and event placement.
- Package distribution/license agreement and supported RotorHazard versions.
- Exact allowed display metadata and renderer capability/version policy from the spike.
- Confirmation of manual snapshot refresh as the event default, including archive/removal behavior.
