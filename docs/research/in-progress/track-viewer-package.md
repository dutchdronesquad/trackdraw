...
- Course-specific assets, copied into the snapshot asset set when supported, not bundled with the installed viewer. Rendering must not depend on an expiring source URL or CDN after import. This class already has a concrete present-day member, not just a future one: MultiGP-branded catalog textures (`organization: "MultiGP"` entries, paths under `multigp-obstacles/`) cannot ship inside the Apache-2.0 viewer package — see the [PVA](../../pva/track-viewer-package-pva.md#phase-0-confirm-distribution-fit) for why — so a design using a MultiGP catalog item must carry those textures in its own snapshot/manual export instead. Where those textures themselves get hosted and maintained is a separate research track: see [Catalog Asset Hosting](../planned/catalog-asset-hosting.md).
